...
namespace LIB_Machine.AdvenTech
{
    public class PCI1730U
    {
        ...
        private const int MAX_INPUT_PORT_NUM = 4;
        private const int MAX_OUTPUT_PORT_NUM = 4;

        public PCI1730U(int deviceNo, int inputStartPort = INPUT_START_PORT, int inputPortNum = MAX_INPUT_PORT_NUM, int outputStartPort = OUTPUT_START_PORT, int outputPortNum = MAX_OUTPUT_PORT_NUM)
        {
        ...
        public bool Open()
        {
            try
            {
                if (InputCtrl.Initialized == false)
                    InputCtrl.SelectedDevice = new DeviceInformation(DeviceNo);
                if (OutputCtrl.Initialized == false)
                    OutputCtrl.SelectedDevice = new DeviceInformation(DeviceNo);
            }
            catch
            {
                return false;
