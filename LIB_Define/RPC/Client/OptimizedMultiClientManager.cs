...
namespace LIB_Define.RPC.Client
{
    ...
    public class OptimizedMultiClientManager : IDisposable
    {
        ...
        private void WireClientEvents(RpcClient client)
        {
            ...
            client.ActionOnReceivedFileFromServer += (idx, path) => OnServerFileReceived?.Invoke(idx, path);
            client.ActionOnReceivedJsonMessageFromServer += (idx, json) => OnServerJsonReceived?.Invoke(idx, json);
            ...
            client.ActionOnReceivedByteDataFromServer += (idx, type, data, metadata) => OnServerByteDataReceived?.Invoke(idx, type, data, metadata);
        ...
        private void UnWireClientEvents(RpcClient client)
        {
            ...
            client.ActionOnReceivedFileFromServer -= OnServerFileReceived;
            client.ActionOnReceivedJsonMessageFromServer -= OnServerJsonReceived;
            client.ActionOnServerImage -= OnServerImageReceived;
            client.ActionOnServerImagePath -= OnServerImagePathReceived;
            client.ActionOnReceivedByteDataFromServer -= OnServerByteDataReceived;
