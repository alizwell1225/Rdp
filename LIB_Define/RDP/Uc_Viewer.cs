...
{
    public event Action<int,bool,bool> ActMaxViewer;
    public event Action<int,bool,bool> ActMidViewer;

    public event Action<int> ActClose;
    ...
    public void Init(int width, int height, int index, int cols, Dictionary<string, RdpConnection> existingConnections)
    {
        ...
        try
        {
            ...
            btnClose.BackColor = ThemeManager.CardColor;
            btnClose.ForeColor = ThemeManager.ForegroundColor;
            btnClose.FlatStyle = FlatStyle.Flat;
            btnClose.FlatAppearance.BorderColor = ThemeManager.BorderColor;
            btnClose.FlatAppearance.MouseOverBackColor = ThemeManager.PrimaryHoverColor;
            btnClose.FlatAppearance.MouseDownBackColor = ThemeManager.PrimaryPressedColor;
    ...
    private void btnFull_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ActMaxViewer?.Invoke(Index,chkTitle.Checked,chkFixSize.Checked);
            splitContainer1.Panel2Collapsed = true;
    ...
    private void btnSmall_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //var frm = this.FindForm() as LIB_RDP.UI.FormRDP;
            //frm?.MaximizeViewer(viewerIndex);
            ActMidViewer?.Invoke(Index, chkTitle.Checked, chkFixSize.Checked);
            splitContainer1.Panel2Collapsed = true;
            //MaximizeRequested?.Invoke(viewerIndex);
        }
        catch (Exception ex)
        {
            MessageBox.Show($"無法縮小視窗: {ex.Message}", "錯誤", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }

    public void SetSmartSize(bool fixSize)
    {
        InnerViewer.GetRdpConnection().SetSmartSizing (fixSize);
    }
}